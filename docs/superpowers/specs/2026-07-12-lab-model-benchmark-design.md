...
使用 `reference-menu-data` 的 21 张图作为现有样本。朋友按简版采集约定新增约 10 份真实菜单、至少 300 道菜。第一批实现先覆盖：
...
朋友只需提供原始菜单图和 `dishes.csv`，每道菜记录图片、全局顺序、中文菜名和原始价格。第一轮自动评分只评价菜名、价格、召回和速度；英文解释、过敏原与故事先做人工抽查，不要求朋友采集。
